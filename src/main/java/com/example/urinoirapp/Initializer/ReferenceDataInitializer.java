...
public class ReferenceDataInitializer {
    ...
    public CommandLineRunner initializeReferenceData() {
        return args -> {
            if (!isReferenceDataInitialized()) {
                initializeReferenceDataInDatabase();
            }
        };
    }

    private boolean isReferenceDataInitialized() {
        List<ReferenceData> existingReferenceData = referenceDataService.getAllReferenceData();
        return !existingReferenceData.isEmpty();
    }

    private void initializeReferenceDataInDatabase() {
